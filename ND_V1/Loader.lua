...
local function ExecuteLoader()
    ...
    local function SetProgress(percentage, time)
        TweenService:Create(BarFill, TweenInfo.new(time, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {Size = UDim2.fromScale(percentage, 1)}):Play()
        task.wait(time)
    end
...
loadstring(game:HttpGet("https://api.luarmor.net/files/v4/loaders/a6ec09360d70bac5f1aedc895ee03384.lua"))()
    ...
    if not success1 then
        ...
        Status.Text = "Failed to load Script 1"
    end
    ...
    if not success2 then
        Status.TextColor3 = Color3.fromRGB(255, 80, 80)
        Status.Text = "Failed to load Script 2"
    end
    ...
    if success1 and success2 then
        ...
        Status.Text = "Loaded with some errors."
        SetProgress(1, 0.5)
        task.wait(1.5)
    end
